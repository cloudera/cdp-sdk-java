/*
 * Copyright (c) 2018 Cloudera, Inc. All Rights Reserved.
 *
 * Portions Copyright (c) Copyright 2013-2018 Amazon.com, Inc. or its
 * affiliates. All Rights Reserved.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 * http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

package com.cloudera.cdp.datahub.model;

import java.util.Objects;
import com.fasterxml.jackson.annotation.JsonProperty;
import com.fasterxml.jackson.annotation.JsonValue;
import com.cloudera.cdp.client.CdpResponse;
import java.util.*;

/**
 * Details for repair instances.
 **/
@jakarta.annotation.Generated(value = "com.cloudera.cdp.client.codegen.CdpSDKJavaCodegen", date = "2025-08-27T10:36:26.277-07:00")
public class RepairInstancesRequest  {

  /**
   * List of instance ids.
   **/
  private List<String> instanceIds = new ArrayList<String>();

  /**
   * The existing disk volumes on the instances will be re-created if the deleteVolumes is true. Otherwise, the volumes will be reattached to the new instances.
   **/
  private Boolean deleteVolumes = null;

  /**
   * Getter for instanceIds.
   * List of instance ids.
   **/
  @JsonProperty("instanceIds")
  public List<String> getInstanceIds() {
    return instanceIds;
  }

  /**
   * Setter for instanceIds.
   * List of instance ids.
   **/
  public void setInstanceIds(List<String> instanceIds) {
    this.instanceIds = instanceIds;
  }

  /**
   * Getter for deleteVolumes.
   * The existing disk volumes on the instances will be re-created if the deleteVolumes is true. Otherwise, the volumes will be reattached to the new instances.
   **/
  @JsonProperty("deleteVolumes")
  public Boolean getDeleteVolumes() {
    return deleteVolumes;
  }

  /**
   * Setter for deleteVolumes.
   * The existing disk volumes on the instances will be re-created if the deleteVolumes is true. Otherwise, the volumes will be reattached to the new instances.
   **/
  public void setDeleteVolumes(Boolean deleteVolumes) {
    this.deleteVolumes = deleteVolumes;
  }

  @Override
  public boolean equals(java.lang.Object o) {
    if (this == o) {
      return true;
    }
    if (o == null || getClass() != o.getClass()) {
      return false;
    }
    RepairInstancesRequest repairInstancesRequest = (RepairInstancesRequest) o;
    if (!Objects.equals(this.instanceIds, repairInstancesRequest.instanceIds)) {
      return false;
    }
    if (!Objects.equals(this.deleteVolumes, repairInstancesRequest.deleteVolumes)) {
      return false;
    }
    return true;
  }

  @Override
  public int hashCode() {
    return Objects.hash(instanceIds, deleteVolumes);
  }

  @Override
  public String toString() {
    StringBuilder sb = new StringBuilder();
    sb.append("class RepairInstancesRequest {\n");
    sb.append("    instanceIds: ").append(toIndentedString(instanceIds)).append("\n");
    sb.append("    deleteVolumes: ").append(toIndentedString(deleteVolumes)).append("\n");
    sb.append("}");
    return sb.toString();
  }

  /**
   * Convert the given object to string with each line except the first indented by 4 spaces.
   */
  private String toIndentedString(java.lang.Object o) {
    if (o == null) {
      return "null";
    }
    return o.toString().replace("\n", "\n    ");
  }
}

