/*
 * Copyright (c) 2018 Cloudera, Inc. All Rights Reserved.
 *
 * Portions Copyright (c) Copyright 2013-2018 Amazon.com, Inc. or its
 * affiliates. All Rights Reserved.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 * http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

package com.cloudera.cdp.lakehouseopt.model;

import java.util.Objects;
import com.fasterxml.jackson.annotation.JsonProperty;
import com.fasterxml.jackson.annotation.JsonValue;
import com.cloudera.cdp.client.CdpResponse;
import com.cloudera.cdp.lakehouseopt.model.DryRunResult;
import com.cloudera.cdp.lakehouseopt.model.ExecutionResult;

/**
 * Response object of policy execution on a table.
 **/
@jakarta.annotation.Generated(value = "com.cloudera.cdp.client.codegen.CdpSDKJavaCodegen", date = "2026-05-01T09:59:31.533-07:00")
public class ExecutePolicyResponse extends CdpResponse {

  /**
   * The dry run result. This field will be present only if the request had dryRun as true.
   **/
  private DryRunResult dryRunResult = null;

  /**
   * The execution result. This field will be present only if the request had dryRun as false.
   **/
  private ExecutionResult executionResult = null;

  /**
   * Getter for dryRunResult.
   * The dry run result. This field will be present only if the request had dryRun as true.
   **/
  @JsonProperty("dryRunResult")
  public DryRunResult getDryRunResult() {
    return dryRunResult;
  }

  /**
   * Setter for dryRunResult.
   * The dry run result. This field will be present only if the request had dryRun as true.
   **/
  public void setDryRunResult(DryRunResult dryRunResult) {
    this.dryRunResult = dryRunResult;
  }

  /**
   * Getter for executionResult.
   * The execution result. This field will be present only if the request had dryRun as false.
   **/
  @JsonProperty("executionResult")
  public ExecutionResult getExecutionResult() {
    return executionResult;
  }

  /**
   * Setter for executionResult.
   * The execution result. This field will be present only if the request had dryRun as false.
   **/
  public void setExecutionResult(ExecutionResult executionResult) {
    this.executionResult = executionResult;
  }

  @Override
  public boolean equals(java.lang.Object o) {
    if (this == o) {
      return true;
    }
    if (o == null || getClass() != o.getClass()) {
      return false;
    }
    ExecutePolicyResponse executePolicyResponse = (ExecutePolicyResponse) o;
    if (!Objects.equals(this.dryRunResult, executePolicyResponse.dryRunResult)) {
      return false;
    }
    if (!Objects.equals(this.executionResult, executePolicyResponse.executionResult)) {
      return false;
    }
    if (!super.equals(o)) {
      return false;
    }
    return true;
  }

  @Override
  public int hashCode() {
    return Objects.hash(dryRunResult, executionResult, super.hashCode());
  }

  @Override
  public String toString() {
    StringBuilder sb = new StringBuilder();
    sb.append("class ExecutePolicyResponse {\n");
    sb.append("    ").append(toIndentedString(super.toString())).append("\n");
    sb.append("    dryRunResult: ").append(toIndentedString(dryRunResult)).append("\n");
    sb.append("    executionResult: ").append(toIndentedString(executionResult)).append("\n");
    sb.append("}");
    return sb.toString();
  }

  /**
   * Convert the given object to string with each line except the first indented by 4 spaces.
   */
  private String toIndentedString(java.lang.Object o) {
    if (o == null) {
      return "null";
    }
    return o.toString().replace("\n", "\n    ");
  }
}

